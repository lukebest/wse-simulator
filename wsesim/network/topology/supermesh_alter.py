"""SuperMeshAlter topology: mesh with alternating peripheral enhanced links."""
    ...
    def _resolve_shape(self, num_nodes: int) -> tuple[int, int]:
        if self.rows is not None and self.cols is not None:
            if self.rows * self.cols != num_nodes:
                raise ValueError("SuperMeshAlter rows*cols must equal num_nodes.")
            return self.rows, self.cols

        side = isqrt(num_nodes)
        if side * side != num_nodes:
            raise ValueError("SuperMeshAlter requires explicit rows/cols for non-square node count.")
        return side, side
