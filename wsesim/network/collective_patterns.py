...
def compare_allgather_patterns(
    rows: int,
    cols: int,
    base_flits: int = 1,
    *,
    ...
    n = rows * cols
    t_star_h1 = optimal_hamilton_makespan(n)
    z_lb_aniso = optimal_z_lower_bound_aniso(
    ...
    if hx == 1 and hy == 1:
        ham_aniso_mk = ham_verify.makespan
    else:
        ham_aniso_flows, ham_aniso_mk, _ = build_hamilton_ring_allgather_flows_aniso(
            rows, cols, hx, hy, orient="auto"
        )
        ham_verify_aniso = verify_preassigned_slots(ham_aniso_flows)
        ham_aniso_mk = (
            ham_aniso_mk
            if ham_verify_aniso.ok
            else calendar_arrival_makespan_aniso(ham_aniso_flows, hx, hy)
        )
    ...
    cal_flows, cal_mk, cal_meta = build_shortest_path_calendar_allgather_flows(
        rows, cols, base_flits, hop_latency_x=hx, hop_latency_y=hy
    )
    cal_verify = verify_preassigned_slots(cal_flows)
    cal_hop = verify_hop_spacing_aniso(cal_flows, hx, hy)
    ...
    return {
        ...
        "node_count": n,
        "t_star": t_star_h1,
        "z_lower_bound_aniso": z_lb_aniso,
        "hx": hx,
        "hy": hy,
        "hamilton": {
            "flow_count": len(ham_flows),
            "makespan": ham_verify.makespan,
            "makespan_aniso": ham_aniso_mk,
        ...
        "calendar": {
            "flow_count": len(cal_flows),
            "makespan": cal_mk,
            "stall": 0,
            "ok": cal_verify.ok and cal_hop,
            "peak": cal_verify.peak if cal_verify.ok else 0,
            "meta": cal_meta,
            "optimal_aniso": cal_mk == z_lb_aniso,
        },
        "speedup_vs_dimwise": (
        ...
        "calendar_vs_hamilton_aniso": (
            ham_aniso_mk / cal_mk if cal_mk and ham_aniso_mk else None
        ),
    }
    ...
    *,
    ...
    builders = {
        ...
        "allgather": lambda: (
            ...
            if variant == "hamilton"
            else build_shortest_path_calendar_allgather_flows(
                rows, cols, flits, hop_latency_x=1, hop_latency_y=1
            )[0]
            if variant == "calendar"
...
def calendar_arrival_makespan_aniso(
    flows: Iterable[CollectiveFlow],
    hx: int,
    hy: int,
) -> int:
    """Latest flit arrival under anisotropic hop latency (last hop + edge latency)."""
    makespan = 0
    for flow in flows:
        if not flow.slots or not flow.path:
            continue
        last = flow.path[-1]
        makespan = max(makespan, flow.slots[-1] + edge_hop_latency(last, hx, hy))
    return makespan


def normalize_calendar_slots_zero(flows: list[CollectiveFlow]) -> int:
    """Shift all slot assignments so the earliest launch is at cycle 0. Returns offset added."""
    min_slot = min(
        slot
        for flow in flows
        if flow.slots
    ...
    for flow in flows:
        if flow.slots is not None:
            flow.slots = [s + offset for s in flow.slots]
    return offset


def build_shortest_path_calendar_allgather_flows(
    rows: int,
    cols: int,
    base_flits: int = 1,
    *,
    ...
    normalize_zero: bool = False,
) -> tuple[list[CollectiveFlow], int, dict[str, int]]:
    """Algorithm B: XY shortest-path allgather with backward JIT slot calendar.

    Schedules N*(N-1) unicasts backward from deadline T* so the farthest flit
    finishes at the §2.2 anisotropic lower bound (max of weighted diameter and
    corner ingress). Edge conflicts are resolved by retiming earlier hops.

    Returns (flows, arrival_makespan, meta). *normalize_zero* shifts slots so
    the earliest injection is at cycle 0 (may increase wall-clock span).
    """
    ...
    t_star = deadline
    if t_star is None:
        t_star = optimal_z_lower_bound_aniso(
            "allgather", rows, cols, hx=hx, hy=hy
        )
    ...
    reserved: set[tuple[str, int]] = set()
    flows: list[CollectiveFlow] = []
    items: list[tuple[int, int, int, int, list[MeshEdge]]] = []
    ...
    for s in range(n):
        ...
        for d in range(n):
            ...
            dx, dy = d % cols, d // cols
            dist = hx * abs(dx - sx) + hy * abs(dy - sy)
            if dist > t_star:
                raise ValueError(
                    f"deadline {t_star} < weighted distance {dist} for {s}->{d}"
                )
            items.append((dist, len(path), s, d, path))

    items.sort(key=lambda it: (-it[0], -it[1], it[2], it[3]))

    cid = 0
    for _dist, _plen, s, d, path in items:
        slots: list[int] = [0] * len(path)
        last_lat = edge_hop_latency(path[-1], hx, hy)
        t = t_star - last_lat
        for hop in range(len(path) - 1, -1, -1):
            edge = path[hop]
            while (edge.id, t) in reserved:
                t -= 1
            slots[hop] = t
            reserved.add((edge.id, t))
            if hop > 0:
                t = slots[hop] - edge_hop_latency(path[hop - 1], hx, hy)
        flows.append(
            CollectiveFlow(
                id=f"cal_{s}_{d}",
                label=f"cal {s}->{d}",
    ...
    time_origin = 0
    if normalize_zero:
        time_origin = -normalize_calendar_slots_zero(flows)

    arrival = calendar_arrival_makespan_aniso(flows, hx, hy)
    min_slot = min(
        slot for flow in flows if flow.slots for slot in flow.slots
    )
    meta = {
        "t_star": t_star,
        "arrival_makespan": arrival,
        "time_origin": time_origin,
        "min_slot": min_slot,
        "span": arrival - min_slot if flows else 0,
        "flow_count": len(flows),
        "hx": hx,
        "hy": hy,
    }
    return flows, arrival, meta
