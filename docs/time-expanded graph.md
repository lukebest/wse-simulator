...
### 9.6 各向异性链路时延（hx ≠ hy，如横向 4 / 纵向 8）

横向（E/W）链路 `hx` cycle、纵向（N/S）链路 `hy` cycle（流水线、II=1 不变）。单 flit 跨 hop 间距约束变为**逐边**的 `t_{j+1} ≥ t_j + h(e_j)`（验证器 `verify_hop_spacing_aniso`）。

**下界**：距离项换成**加权曼哈顿度量** `dist_w = hx·|Δx| + hy·|Δy|`，带宽项仍不缩放（`optimal_z_lower_bound_aniso()`）：

- 加权离心率 `ecc_w(r) = max_v (hx|Δx|+hy|Δy|)`，加权直径 `D_w = hx(X−1) + hy(Y−1)`；
- **AllGather**：`z* = max(D_w, ⌈(N−1)/2⌉)`；**AllToAll**：`z* = max(⌈NX/4⌉, ⌈NY/4⌉, D_w)`——bisection 是逐链路计数，**与时延无关**；
- hx=4, hy=8：4×4 `D_w=36`、8×8 `D_w=84`、12×16 `D_w=148`。

**AllGather：环时序与环走向（重点 1）**

均匀膨胀（×max(hx,hy)）浪费便宜的横边。正确做法是**累积加权时序**（`build_hamilton_ring_allgather_flows_aniso`）：flit 到达即转发，第 k 跳的发射时刻 = 前 k 条环边时延的累积和。冲突自由证明：同一条有向环边上，各 flit 走过的是「以该边结尾的 k 条连续边」（k=1,2,…），发射时刻是长度递增的后缀和（每项 ≥1），严格递增 ⇒ 两两不同。

新自由度：**环的走向**。蛇形环沿哪个维度扫，决定环上贵边/便宜边的占比——`hy > hx` 时应**沿便宜的横向蛇形**（row snake），让环主要由 hx 边构成。实测（hx=4, hy=8，到达时刻；下界 `max(D_w,⌈(N−1)/2⌉)`）：

| 规模 | 纵蛇形（贵） | **横蛇形（便宜）** | 提升 | `z*` |
|------|-------------|------------------|------|------|
| 4×4 | 56 | **48** | 1.17× | 36 |
| 8×8 | 240 | **168** | 1.43× | 84 |
| 12×16 | 736 | **448** | 1.64× | 148 |

横蛇形环约 `⌈(N−1)/2⌉·h̄`（h̄ ≈ 环上加权平均时延 → hx），相对下界仍有 §9.5 所述单 flit 串行 gap；`F ≥ h̄` 的大块同样摊销回带宽口径。

**AllToAll：逐轴跨步装配（重点 2）**

二相构造天然按维分相：X 相全为横边、Y 相全为纵边，故 first-fit 改为**逐轴步长**——X 相检查 `φ, φ+hx, φ+2hx, …`，Y 相 `φ, φ+hy, …`（`build_alltoall_twophase_flows(hop_latency_x=4, hop_latency_y=8)`）。同链路不同 flit 仍 1/cycle 密排，带宽项不受 (hx,hy) 影响。实测：

| 规模 | `Z_1`（hx=hy=1） | `Z`（4/8） | 增量 | `z*`（4/8） | 比值 |
|------|-----------------|-----------|------|-------------|------|
| 4×4 | 44 | **61** | +17 | 36 | ≈1.9× |
| 8×8 | 272 | **305** | +33 | 128 | ≈2.4× |
| 12×16 | 1366 | **1425** | +59 | 768 | ≈1.9× |

增量 ≈ `hx·X + hy·Y` 量级的加性尾巴（X 相拉长 `(X−1)(hx−1)`、Y 相拉长 `(Y−1)(hy−1)`，再加 PE 屏障平移），乘性带宽项原封不动。全部经 `verify_preassigned_slots` + `verify_hop_spacing_aniso` 验证（`ok, peak=1, stall=0`）。
